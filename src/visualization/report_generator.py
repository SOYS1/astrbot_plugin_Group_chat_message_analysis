...
class ReportGenerator:
    ...
    def _generate_html_report(self, analysis_result: Dict, keyword: str, group_id: str) -> str:
        """
        生成HTML格式的报告
        """
        # 准备数据
        ...
        # 生成主要贡献者HTML
        top_contributors_html = self._generate_top_contributors(analysis_result['top_contributors'])
        
        # 生成关键讨论内容HTML
        key_discussions_html = self._generate_key_discussions(analysis_result['key_discussions'])
        
        # 生成讨论会话HTML（双排并列布局）
        discussion_sessions_html = self._generate_discussion_sessions(analysis_result['discussion_sessions'])
                ...
                <div class="content">
                    <div class="section">
                        ...
                        <div class="stats-grid">
                            <div class="stat-card">
                                <div class="stat-number">{analysis_result['total_messages']}</div>
                                <div class="stat-label">相关消息数</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-number">{len(analysis_result['participants'])}</div>
                                <div class="stat-label">参与人数</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-number">{len(analysis_result['top_contributors'])}</div>
                                <div class="stat-label">活跃用户</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-number">{len(analysis_result['discussion_sessions'])}</div>
                    ...
                    <div class="summary-box">
                        ...
                        <div class="summary-content">
                            {analysis_result.get('topic_summary', f'关于"{keyword}"话题的分析结果')}
                        </div>
                    </div>
    ...
    def _generate_top_contributors(self, top_contributors: Dict) -> str:
        """
        生成主要贡献者HTML
        """
    ...
    def _generate_key_discussions(self, key_discussions: List[Dict]) -> str:
        """
        生成关键讨论内容HTML（缩小卡片尺寸）
        """
    ...
    def _generate_discussion_sessions(self, discussion_sessions: List[Dict]) -> str:
        """
        生成讨论会话HTML（双排并列布局）
        """
